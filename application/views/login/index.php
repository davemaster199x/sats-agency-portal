...
echo form_open('login/index',$form_attr); 
?>
		
	<div class="text-center">
	
		<img src="<?= theme('/images/login_logo.png') ?>" class="<?= $this->config->item('theme') ?> logo login_logo" alt="logo"/>
	</div>
...
<?php 
		// 2FA code expired
		if( $this->session->flashdata('user_2fa_code_expired') == 1 ){ ?>
			<div class="alert alert-danger">
			Code Expired
			</div>
		<?php
...
<?php if(!empty($this->input->get_post('crm_login'))): ?>
<script>
jQuery(document).ready(function(){
    jQuery("#sign_in_btn").click();
});
</script>
<?php endif; ?>