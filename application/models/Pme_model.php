...
class Pme_model extends CI_Model
{
    ...
    public function send_all_certificates_and_invoices($is_get_data = false) {
        ...
        $custom_where = "( j.`is_pme_invoice_upload` IS NULL AND j.`is_pme_bill_create` IS NULL AND ( j.`client_emailed` IS NULL || j.`client_emailed` = '' ) ) AND (p.`propertyme_prop_id` IS NOT NULL AND p.`propertyme_prop_id` != '') AND (a.pme_supplier_id IS NOT NULL AND a.pme_supplier_id <> '') AND (aat.connection_date IS NOT NULL AND aat.connection_date != '') AND p.`send_to_email_not_api` = 0";
        $paramsPmeSent = array(
        ...
        foreach ($listsPme as $val) {
            ...
            $upload_inv_params = array(
                'propertyme_prop_id' => $val['propertyme_prop_id'],
                'a_id' => $val['a_id'],
                'jid' => $val['jid'],
            );
    ...
    public function send_all_certificates_and_invoices_via_vjd($job_id_by_vjd = "") {
        ...
        $custom_where = "
        (p.`propertyme_prop_id` IS NOT NULL AND p.`propertyme_prop_id` != '') AND 
    ...
    public function upload_pdf_to_property($agencyId, $pmePropId, $file, $fileName, $job_details) {
        ...
        // job log
        $encrypt = rawurlencode($this->hashIds->encodeString($job_id));
    ...
    public function upload_pdf_to_bill($agencyId, $pmePropId, $jobId, $countryId, $job_details, $property_details, $alarm_details, $num_alarms) {
        ...
        if (!is_null($suppId->pme_supplier_id) && floatval($getTotalAmount) > 0) {
            ...
            if( $bill_error == true || $property_details['send_to_email_not_api'] == 1 ){ // bill creation will fail on PMe, send invoice to email instead
                ...
                // job log1
                $encrypt = rawurlencode($this->hashIds->encodeString($jobId));
                $baseUrl = $_SERVER["SERVER_NAME"];
                if(isset($_SERVER['HTTPS'])){
                    $protocol = ($_SERVER['HTTPS'] && $_SERVER['HTTPS'] != "off") ? "https" : "http";
                } else{
                ...
                $log_details = "<a href='".$protocol."://{$baseUrl}/pdf/invoices/{$encrypt}'>Invoice</a>, #{$invoice_number} uploaded to PMe Agency as a bill of {$getTotalAmount}";
                $log_params = array(
                    'title' => 69,  // PMe API
                    'details' => $log_details,
                    'display_in_vjd' => 1,
